...
function JMTemplate({ subUrl, btnLabel, type, field }) {
	...
	const [showUnderline, setShowUnderline] = useState(() => {
		const storedValue = localStorage.getItem("showUnderline");
		return storedValue ? JSON.parse(storedValue) : true;
	});
	// const [showFurigana, setShowFurigana] = useState(true);
	const [showRomaji, setShowRomaji] = useState(() => {
		const storedValue = localStorage.getItem("showRomaji");
		return storedValue ? JSON.parse(storedValue) : true;
	});
	...
	useEffect(() => {
		if (startLoading) {
			const t = setTimeout(() => setShowStuckWarning(true), 3000);
	...
	const formatData = async (d) => {
		...
		for (const sentence of d) {
			...
			const fparts_skeleton = {
				key: sentence.jp_id,
				structs: structs,
			};
			structs.forEach((sct, i) => {
			...
			//           {glosses.map((g, z) => {
			//             return (
			//               <span key={`sp-m-${z}`} className="col-auto mm mt-1 mb-2">
			//                 {g.trim()}
			//               </span>
			//             );
			//           })}
			...
			//                   key={`span-container-${word}-${i}`}
			//                 >
			//                   {/* furigana  / romaji */}
			//                   <span
			//                     className="readings mt-2"
			//                     key={`readings-${word}-${i}`}
			...
			const fkanji = (
				<Box>
					{sentence.kanji.map((k, i) => {
						...
						return (
							<React.Fragment key={"kFragment " + i}>
								<Box key={`li-${k}`} className="p-1">
									<div
										key={`div-${k}`}
										className="section p-2"
									>
										<Heading
											key={`sp-ele-${k}`}
											className="text"
			...
			new_kjs.push(fkanji);
			new_structSentence.push(fparts_skeleton);
		}
		if (new_ids.length) {
	...
	const renderSentenceSkeleton = () => {
		let key = structSentences[itemIndex].key;
		let structs = structSentences[itemIndex].structs;
		return (
			<ul key={key} className="sentence" style={{ paddingLeft: "0" }}>
				{structs.map((sct, i) => {
					// 1st layer CONTENT
					const word = sct.word;
					const hiragana = sct.reading_hiragana;
					const hasParts =
						word !== sct.reading_katakana && word !== hiragana;
						...
						<section className="row container">
							{glosses.map((g, z) => {
								return (
									<span
										key={`sp-m-${z}`}
										className="col-auto mm mt-1 mb-2"
									>
										{g.trim()}
									</span>
								);
	...
	const fetchRandom = async (n) => {
		...
		if (type === "custom") {
			...
			res = await fetch(
				`https://remonapi.vercel.app/api/${n}/${field}/${quizFreq}/${quizCount}`
			);
			isValid = true;
		} else if (type === "random") {
		...
		if (isValid && res && type) {
			...
			alert(`Invalid link or sentence generation`);
		}
	...
	return (
		<div style={{ height: "100vh" }}>
			...
			>
				<div width={"100%"}>
					{typeof structSentences[itemIndex] === "undefined" &&
					!startLoading ? (
						...
						<VStack>
							...
							{showStuckWarning && (
								<Text
									...
									fontWeight={600}
								>
									Stuck loading? please refresh the website if
			...
			<Box className="mb-2 mt-5" style={{ height: "30%" }}>
				...
				<Box>
					<Menu>
						<MenuButton as={Button} variant="solid" className="m-1">
							Features
						</MenuButton>
						<MenuList>
							<MenuGroup title="Settings">
								...
								<MenuItem as="div">
									<Checkbox
										isChecked={showUnderline}
										onChange={() =>
											setShowUnderline((prev) => {
												let newVal = !prev;
												localStorage.setItem(
													"showUnderline",
													JSON.stringify(newVal)
